 ...
 */
 ...
 bool SerialCommand::addCommand(const String& name, const String& params, const String& description, CommandHandler handler) {
 ...
 bool SerialCommand::handleStatus(const String& params, String& response) {
     ...
     if (safetyMonitor != nullptr) {
         ...
         switch (safetyStatus) {
             ...
             case SystemSafetyStatus::EMERGENCY_STOP: safetyStr = "E-Stop"; break;
 ...
 bool SerialCommand::handleMove(const String& params, String& response) {
     ...
     if (velocity > 0.0f && accel > 0.0f) {
         ...
         motor->moveToPosition(position, velocity, CONFIG_DEFAULT_ACCELERATION, CONFIG_DEFAULT_DECELERATION, 0.0f);
     } else {
 ...
 bool SerialCommand::handleStop(const String& params, String& response) {
     ...
     if (paramsStr.length() == 0) {
         ...
         if (paramsStr.length() == 0) {
             ...
             response = emergency ? 
                        "Emergency stopped motor " + paramsStr :
 ...
 bool SerialCommand::handleHome(const String& params, String& response) {
     ...
     response = "Homing motor " + indexStr + " in " + (direction > 0 ? "positive" : "negative") + " direction";
 ...
 bool SerialCommand::handleDebug(const String& params, String& response) {
     ...
     if (paramsStr.length() == 0) {
         ...
         if (level >= 0 && level <= 5) {
             ...
             response = "Invalid log level. Valid levels: 0=OFF, 1=ERROR, 2=WARNING, 3=INFO, 4=DEBUG, 5=VERBOSE";
             return false;
         }
     }
 }
