 ...
 */
 ...
 StatusReporter::StatusReporter(SystemManager* systemManager, uint8_t updateFrequencyHz)
     ...
     , m_updateFrequencyHz(updateFrequencyHz > 0 ? updateFrequencyHz : CONFIG_STATUS_UPDATE_FREQUENCY_HZ)
     , m_lastUpdateMs(0)
 ...
 void StatusReporter::collectStatusData() {
     ...
     if (taskScheduler != nullptr) {
         uint32_t controlTaskCount, auxiliaryTaskCount, totalMissedDeadlines, averageControlLoopTimeUs;
         taskScheduler->getSchedulerStats(controlTaskCount, auxiliaryTaskCount, totalMissedDeadlines, averageControlLoopTimeUs);
