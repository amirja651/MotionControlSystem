 ...
 */
 ...
 void SafetyMonitor::setVoltageTolerances(float minVoltage, float maxVoltage, float warningThreshold) {
     m_minVoltage = minVoltage;
     m_maxVoltage = maxVoltage;
     m_voltageWarningThreshold = warningThreshold;
 }
 
 void SafetyMonitor::getSafetyStats(uint32_t& warningCount, uint32_t& errorCount, uint32_t& emergencyStopCount) {
 ...
 SafetyCode SafetyMonitor::checkMotorSafety() {
     ...
     for (uint8_t i = 0; i < m_motorManager->getMotorCount(); i++) {
         Motor* motor = m_motorManager->getMotor(i);
         if (motor != nullptr && motor->isEnabled() && motor->getControlMode() == MotorControlMode::POSITION) {
     ...
     for (uint8_t i = 0; i < m_motorManager->getMotorCount(); i++) {
         Motor* motor = m_motorManager->getMotor(i);
         if (motor != nullptr && motor->isEnabled() && motor->getControlMode() == MotorControlMode::VELOCITY) {
