 ...
 */
 ...
 Motor::Motor(const MotorConfig& config)
     ...
     , m_trajectoryPlanner(config.maxVelocity, config.maxAcceleration, config.maxDeceleration, config.maxJerk)
     , m_driver(nullptr)
 ...
 bool Motor::initialize() {
     ...
     m_driver = new StepperDriver(m_config.stepPin, m_config.dirPin, m_config.enablePin, m_config.invertDirection);
     ...
     if (gpioManager != nullptr) {
         if (m_config.limitMinPin != 0xFF) {
             gpioManager->allocatePin(m_config.limitMinPin, PinMode::INPUT_PULLUP_PIN, "Motor" + String(m_config.index) + "LimitMin");
         }
         
         if (m_config.limitMaxPin != 0xFF) {
             gpioManager->allocatePin(m_config.limitMaxPin, PinMode::INPUT_PULLUP_PIN, "Motor" + String(m_config.index) + "LimitMax");
         }
     }
 ...
 void Motor::updateMotorState() {
     ...
     if (m_state.status == MotorStatus::MOVING) {
         ...
         if (m_controlMode == MotorControlMode::POSITION) {
             ...
             if (m_state.targetVelocity == 0.0f && fabs(m_state.currentVelocity) < m_velocityTolerance) {
                 m_state.status = MotorStatus::IDLE;
             }
         }
     }
