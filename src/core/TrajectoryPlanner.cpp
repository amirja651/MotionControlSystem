 ...
 */
 ...
 bool TrajectoryPlanner::planPositionMove(int32_t currentPosition, int32_t targetPosition,
     ...
     if (profileType == ProfileType::S_CURVE) {
         m_state.profileDuration = calculateSCurveProfile(distance, currentVelocity, 0.0f, maxVelocity, 
                                                        maxAcceleration, maxDeceleration, maxJerk);
     } else {
         // Default to trapezoidal
         m_state.profileDuration = calculateTrapezoidalProfile(distance, currentVelocity, 0.0f, maxVelocity, 
 ...
 bool TrajectoryPlanner::planVelocityMove(float currentVelocity, float targetVelocity,
     ...
     m_state.maxVelocity = fabs(targetVelocity); // For velocity move, max velocity is target velocity
     m_state.maxAcceleration = maxAcceleration;
 ...
 void TrajectoryPlanner::setMaxAcceleration(float maxAcceleration) {
     m_defaultMaxAcceleration = maxAcceleration > 0.0f ? maxAcceleration : CONFIG_DEFAULT_ACCELERATION;
 }
 
 void TrajectoryPlanner::setMaxDeceleration(float maxDeceleration) {
     m_defaultMaxDeceleration = maxDeceleration > 0.0f ? maxDeceleration : CONFIG_DEFAULT_DECELERATION;
 }
 ...
 float TrajectoryPlanner::calculateTrapezoidalProfile(float distance, float initialVelocity, float targetVelocity,
     ...
     float accelDistance = initialVelocity * accelTime + 0.5f * maxAcceleration * accelTime * accelTime;
     ...
     if (cruiseDistance < 0.0f) {
         ...
         while (cruiseDistance < 0.0f && step > 0.001f) {
             ...
             accelDistance = initialVelocity * accelTime + 0.5f * maxAcceleration * accelTime * accelTime;
             decelDistance = adjustedMaxVelocity * decelTime - 0.5f * maxDeceleration * decelTime * decelTime;
         ...
         if (cruiseDistance < 0.0f) {
             ...
             if (initialVelocity == 0.0f && targetVelocity == 0.0f) {
                 float peakVelocity = sqrt(2.0f * maxAcceleration * maxDeceleration * distance / (maxAcceleration + maxDeceleration));
 ...
 float TrajectoryPlanner::calculateSCurveProfile(float distance, float initialVelocity, float targetVelocity,
                                              float maxVelocity, float maxAcceleration, float maxDeceleration, float maxJerk) {
     ...
     float trapezoidalDuration = calculateTrapezoidalProfile(distance, initialVelocity, targetVelocity,
 ...
 void TrajectoryPlanner::updateSCurveProfile(float elapsedTimeSeconds) {
     ...
     m_state.currentAcceleration = (positionNext - 2.0f * positionCurrent + positionPrev) / (dt * dt);
 }
 // End of Code