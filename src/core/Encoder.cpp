 ...
 */
 ...
 void Encoder::update(uint32_t deltaTimeUs) {
     ...
     if (elapsedTimeUs > 0) {
         ...
         m_filteredVelocity = MathUtils::lowPassFilter(m_filteredVelocity, newVelocity, m_velocityFilterAlpha);
         ...
         m_filteredPosition = MathUtils::lowPassFilter(m_filteredPosition, static_cast<float>(m_position), m_positionFilterAlpha);
