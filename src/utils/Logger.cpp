...
Logger::Logger()
    ...
    m_moduleEnabled[static_cast<size_t>(LogModule::COMMAND_HANDLER)] =
        CONFIG_LOG_COMMANDHANDLER_ENABLED;
...
// Additionally, check the log function for potential race conditions:
void Logger::log(LogLevel level, const String& message, LogModule module) {
    ...
    // Queue for async processing - add mutex protection if needed
...
void Logger::outputLogEntry(const LogEntry& entry) {
    ...
    if (m_serialOutputEnabled && Serial) {
        // Ensure the message ends with a proper line break
        Serial.println(formattedMessage);  // Use println instead of print
        // Make sure any buffers are flushed
        Serial.flush();  // Add this to ensure complete transmission
    }
