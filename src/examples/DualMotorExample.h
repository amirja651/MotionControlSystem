 ...
 */
 ...
 private:
     ...
     void configureSafety() {
         ...
         safetyMonitor->setVelocityTolerances(80.0f, 400.0f);  // 80 steps/s warning, 400 steps/s error
     ...
     void configureCommands() {
         ...
         serialCommand->addCommand("home_axis", "<axis> [direction]", "Home specific axis (x or y)",
             [this](const String& params, String& response) {
                 return homeAxis(params, response);
             });
         ...
         serialCommand->addCommand("rectangle", "<width> <height> [speed]", "Move in rectangular pattern",
             [this](const String& params, String& response) {
                 return moveRectanglePattern(params, response);
             });
             
         serialCommand->addCommand("circle", "<radius> [speed] [segments]", "Move in circular pattern",
             [this](const String& params, String& response) {
     ...
     bool homeAllAxes(const String& params, String& response) {
         ...
         response = "Homing all axes in " + String(direction > 0 ? "positive" : "negative") + " direction";
         return true;
     }
     ...
     bool homeAxis(const String& params, String& response) {
         ...
         response = "Homing " + axisStr + " axis in " + String(direction > 0 ? "positive" : "negative") + " direction";
         return true;
     }
     ...
     bool moveToPosition(const String& params, String& response) {
         ...
         motorManager->moveMultipleMotors(positions, motorIndices, 2, speed, speed * 2.0f, speed * 2.0f);
     ...
     bool moveRectanglePattern(const String& params, String& response) {
         ...
         response = "Moving in rectangle pattern of width " + widthStr + 
                   ", height " + heightStr + " at speed " + speedStr;
         ...
         if (scheduler != nullptr) {
             scheduler->registerAuxiliaryTask([this, xMotor, yMotor, x1, y1, x2, y2, x3, y3, x4, y4, speed]() {
      ...
      */
     void executeRectangleMotion(Motor* xMotor, Motor* yMotor, 
     ...
     bool moveCirclePattern(const String& params, String& response) {
         ...
         response = "Moving in circular pattern with radius " + radiusStr + 
                   " at speed " + speedStr + " using " + segmentsStr + " segments";
         
         // Execute circle motion in a separate task to avoid blocking
         TaskScheduler* scheduler = m_systemManager->getTaskScheduler();
         if (scheduler != nullptr) {
             scheduler->registerAuxiliaryTask([this, xMotor, yMotor, xCenter, yCenter, radius, speed, segments]() {
      ...
      */
     void executeCircleMotion(Motor* xMotor, Motor* yMotor, 
