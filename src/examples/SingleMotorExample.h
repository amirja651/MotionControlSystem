 ...
 */
 ...
 private:
     ...
     void configureSafety() {
         ...
         safetyMonitor->setVelocityTolerances(50.0f, 300.0f);  // 50 steps/s warning, 300 steps/s error
     ...
     void configureCommands() {
         ...
         serialCommand->addCommand("home", "[direction]", "Home the motor",
             [this](const String& params, String& response) {
                 return homeMotor(params, response);
             });
         
         serialCommand->addCommand("move_rel", "<distance> [speed]", "Move motor by relative distance",
             [this](const String& params, String& response) {
                 return moveRelative(params, response);
             });
             
         serialCommand->addCommand("move_to", "<position> [speed]", "Move motor to absolute position",
             [this](const String& params, String& response) {
                 return moveAbsolute(params, response);
             });
             
         serialCommand->addCommand("oscillate", "<amplitude> <frequency> [cycles]", "Oscillate motor",
             [this](const String& params, String& response) {
     ...
     bool homeMotor(const String& params, String& response) {
         ...
         response = "Homing motor in " + String(direction > 0 ? "positive" : "negative") + " direction";
         return true;
     }
     ...
     bool oscillateMotor(const String& params, String& response) {
         ...
         response = "Oscillating motor with amplitude " + amplitudeStr + 
                   ", frequency " + frequencyStr + "Hz, " + cyclesStr + " cycles";
         
         // Execute oscillation in a separate task to avoid blocking
         TaskScheduler* scheduler = m_systemManager->getTaskScheduler();
         if (scheduler != nullptr) {
             scheduler->registerAuxiliaryTask([this, motor, centerPosition, amplitude, speed, accel, cycles]() {
      ...
      */
     void executeOscillation(Motor* motor, int32_t centerPos, int32_t amplitude, 
                             float speed, float accel, int cycles) {
