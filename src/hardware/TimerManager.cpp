 ...
 */
 ...
 bool TimerManager::startTimer(uint8_t timerIndex, uint32_t intervalUs, bool autoReload, TimerCallback callback) {
     // Validate parameters
 ...
 bool TimerManager::setTimerInterval(uint8_t timerIndex, uint32_t intervalUs) {
     ...
     if (m_timerRunning[timerIndex]) {
         ...
         return startTimer(timerIndex, intervalUs, m_timerAutoReload[timerIndex], m_timerCallbacks[timerIndex]);
     }
 ...
 bool TimerManager::configureTimer(uint8_t timerIndex, uint32_t intervalUs, bool autoReload) {
     ...
     if (timerIndex == 0) {
         timer_isr_register(m_timerGroup, m_timerIdx[timerIndex], timer0ISR, nullptr, ESP_INTR_FLAG_IRAM, nullptr);
     } else {
         timer_isr_register(m_timerGroup, m_timerIdx[timerIndex], timer1ISR, nullptr, ESP_INTR_FLAG_IRAM, nullptr);
     }
